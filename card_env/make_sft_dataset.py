...
def make_env(env_config, language_only=False):
    def _init():
...
env_fns = [make_env(configs.env_config, language_only=False) for _ in range(num_envs)]
...
samples = []
img_out_dir = "/home/suganuma/datasets/card_24/sft/images"
os.makedirs(img_out_dir, exist_ok=True)
os.makedirs("/home/suganuma/datasets/card_24/sft/train", exist_ok=True)
...
for i in tqdm(range(sample_num), desc="Generating SFT dataset"):
    ...
    for env_idx in range(num_envs):
        ...
        if len(samples) % 5000 == 0:
            with open(os.path.join("/home/suganuma/datasets/card_24/sft/train", "sft_data.json"), "w") as f:
