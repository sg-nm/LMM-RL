...
def train(args):
    ...
    bundles = [{"GPU": 1, "CPU": 1} for _ in range(args.ui_tars_num_engines * args.ui_tars_tensor_parallel_size)]
    pg_ui_tars = placement_group(bundles, strategy="PACK")
    ray.get([pg_ui_tars.ready()])
    ui_tars_engines = None
    if args.ui_tars_num_engines is not None and args.ui_tars_num_engines > 0:
        ui_tars_engines = create_vllm_engines(
            args.ui_tars_num_engines,
            args.ui_tars_tensor_parallel_size,
            args.ui_tars_model,
            args.seed,
            args.enable_prefix_caching,
            args.enforce_eager,
    ...
    if not args.remote_rm_url and reward_models is not None:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--feedback_vllm_num_engines", type=int, default=1, help="number of vLLM Engines, set to 0 to disable vLLM")
    parser.add_argument("--feedback_vllm_tensor_parallel_size", type=int, default=1, help="tensor parallel size of vLLM Engine for multi-GPU inference")
    parser.add_argument("--feedback_model", type=str, default=None, help="HF model name or path")
    parser.add_argument("--grounding_num_engines", type=int, default=1, help="number of vLLM Engines, set to 0 to disable vLLM")
    parser.add_argument("--grounding_tensor_parallel_size", type=int, default=1, help="tensor parallel size of vLLM Engine for multi-GPU inference")
    parser.add_argument("--grounding_model", type=str, default=None, help="HF model name or path")
    ...
    parser.add_argument("--n_samples_per_prompt", type=int, default=1, help="number of responses for each prompt in generation")
    parser.add_argument("--n_feedback_samples_per_prompt", type=int, default=8, help="number of responses for each prompt in feedback generation")
    ...
    # Custom dataset
    parser.add_argument("--dataset_name", type=str, default="card_24", help="HF dataset name or path")
    ...
    parser.add_argument("--wandb_project", type=str, default="TextGrad")
    parser.add_argument(
        "--wandb_run_name",
        type=str,
    ...
    parser.add_argument("--reasoning_logprob_weight", type=float, default=1.0, help="weight for reasoning logprob")
    ...
    parser.add_argument("--vm_config", type=str, default="vmconfig.yaml", help="VM config file")
