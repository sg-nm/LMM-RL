...
class Actor(nn.Module):
    ...
    def forward(
        ...
        if not self.packing_samples:
            ...
            action_log_probs = []
            offset = 0
            for num_action, seq_len in zip(action_mask.shape[1], packed_seq_lens):
...
class MultiModalActor(nn.Module):
    ...
    def process_sequences(self, sequences: torch.Tensor, input_len, eos_token_id, pad_token_id):
        ...
        - sequences [B, L]: the sequences with eos_token_id added
        - attention_mask [B, L]: 0 for padding and eos toknes, 1 for other tokens (i.e., valid tokens including input prompts)
        - action_mask [B, L-input_len]: 0 for padding and eos toknes, 1 for generated tokens by an actor (i.e., only generated action tokens)
        """
        attention_mask = (sequences.ne(eos_token_id) & sequences.ne(pad_token_id)).to(dtype=torch.long)
        seq_length = attention_mask.size(1)
        ...
        # For Llama3 and Qwen2 models, there are some eos_tokens in the middle of the prompt.
        # Thus we find the last valid token in the prompt and insert the eos_token after it, and finally mask the subsequent tokens.
        first_token_indices = attention_mask.long().argmax(dim=1, keepdim=True)
        mask = torch.arange(seq_length).unsqueeze(0).expand(sequences.size(0), -1).to(device=sequences.device)
        attention_mask = (mask >= first_token_indices) & (mask <= eos_indices).to(dtype=torch.long)
    ...
    def process_sequence(sequence: torch.Tensor, input_len, eos_token_id, pad_token_id):
        """
        This function outputs the following for a single sequence (not batched):
        - sequence [L]: the sequence with eos_token_id added
        - attention_mask [L]: 0 for padding and eos tokens, 1 for other tokens (i.e., valid tokens including input prompts)
        - action_mask [L-input_len]: 0 for padding and eos tokens, 1 for generated tokens by an actor (i.e., only generated action tokens)
        """
        # Create attention mask
        attention_mask = (sequence.ne(eos_token_id) & sequence.ne(pad_token_id)).to(dtype=torch.long)
        seq_length = attention_mask.size(0)
        
        # Find the last valid token (non-pad, non-eos) and put an EOS token after it
        # The following is equivalent to searching from the end to find the first valid token
        eos_index = seq_length - attention_mask.long().flip(0).argmax(dim=0).clamp(min=1)
        sequence[eos_index] = eos_token_id
        
        # Find the first valid token
        first_token_index = attention_mask.long().argmax(dim=0)
        
        # Create mask for tokens between first valid token and eos token
        mask = torch.arange(seq_length, device=sequence.device)
        attention_mask = ((mask >= first_token_index) & (mask <= eos_index)).to(dtype=torch.long)
        
        # Create action mask for generated tokens
        state_seq = sequence[input_len - 1 : -1]
        action_mask = state_seq.ne(eos_token_id) & state_seq.ne(pad_token_id)
        
        # Ensure the first token after input is always considered an action
        if len(action_mask) > 0:
            action_mask[0] = 1
        
        return sequence, attention_mask, action_mask
