...
from .ring_attn_utils import convert_ring_attn_params, set_hacked_position_ids, clear_hacked_position_ids, unpad_and_slice_tensor, gather_and_pad_tensor
...
#     """
#     assert (model_type == "critic" or model_type == "reward"), f"invalid model_type: {model_type}, should be critic or reward."
...
#         else:
#             cls_class = _get_critic_model_language(base_pretrained_class, base_class, value_head_prefix, packing_samples)
...
# Language
def _get_critic_model_language(base_pretrained_model, base_llm_model, value_head_prefix="score", packing_samples=False):
    class CriticModel(base_pretrained_model):
        ...
        def forward(
            ...
            outputs = getattr(self, self.base_model_prefix)(
                input_ids, attention_mask=forward_attention_mask, position_ids=position_ids, output_hidden_states=True
            )
            ...
            if action_mask is None:
                ...
                # last_token_indices = seq_length - attention_mask.long().fliplr().argmax(dim=1, keepdim=True).clamp(min=1)
                # sequence_lengths = attention_mask.sum(dim=1)
                # last_token_indices = sequence_lengths - 1
                # state_representation shape: (batch_size, hidden_size)
                # state_representation = last_hidden_states[torch.arange(last_hidden_states.shape[0], device=input_ids.device), last_token_indices, :]
                state_representation = last_hidden_states[torch.arange(last_hidden_states.shape[0], device=input_ids.device), -1, :]
                values = getattr(self, self.value_head_prefix)(state_representation)  # (batch_size, 1)
                if return_output:
                ...
                raise NotImplementedError("Action mask is not implemented for critic model of language model")
                # last_hidden_states = outputs["hidden_states"][-1]
