...
from openrlhf.trainer.ppo_utils import Experience, RemoteExperienceMaker_GUI
...
from gui_env.config_dataclass import VMConfig, load_config_from_yaml
...
class ActorModelRayActor_GUI(BasePPORole):
    def init_model_from_pretrained(self, strategy: DeepspeedStrategy, pretrain):
        ...
        if args.grounding_model:
            self.grounding_data_processor = AutoProcessor.from_pretrained(args.grounding_model, padding_side="left")
        else:
            self.grounding_data_processor = None
    ...
    def prepare_env(self):
        ...
        self.configs = load_config_from_yaml(args.vm_config)

        with open(self.configs.task_configs_file, "r") as f:
            task_configs = json.load(f)
        random.shuffle(task_configs)
        print(f"Generating {self.configs.num_envs} envs...")
        self.parallel_env = ParallelDesktopEnv(
            num_containers=self.configs.num_envs,
            docker_image_name=self.configs.docker_image_name,
            host_task_dir=self.configs.host_task_dir,
            action_space=self.configs.action_space,
            task_config_list=task_configs,
    ...
    def fit(
        ...
        trainer = ActorPPOTrainer_GUI(
            ...
            grounding_data_processor=self.grounding_data_processor,
...
class ActorPPOTrainer_GUI(PPOTrainer):
    def __init__(
        ...
        remote_rm_url: List[str] = None,
        critic_train_remote: bool = False,
        envs: ParallelDesktopEnv = None,
        eval_envs: ParallelDesktopEnv = None,
        **kwargs,
    ):
        ...
        ppo_grounding_data_processor = kwargs.pop('grounding_data_processor', None)
        ...
        self.grounding_data_processor = ppo_grounding_data_processor
        # override replay buffer
        packing_samples = getattr(self.strategy.args, "packing_samples", False)
        self.replay_buffer = ReplayBuffer_GUI(
        ...
        if self.strategy.args.advantage_estimator == "reinforce" or self.strategy.args.advantage_estimator == "gae":
                self.experience_maker = RemoteExperienceMaker_GUI_REINFORCE(
                actor=self.actor,
                critic=self.critic,
